...
class Generative():
    ...
    def updates(self, u, lambda_p, lambda_1, lambda_2, lambda_3, pos, sample):
        ...
        partial_X_b = np.array([([0.]* self.itemNum) for i in range(self.itemNum)])
        
        sample_list=list(sample)
        for i in range(self.itemNum):
            partial_X_b[i][sample] = (-exp_ratings[i] * exp_ratings[sample]) / sum_expr_square
            occur_num=sample_list.count(i)#to count the occur number of item i
            if occur_num !=0 :
                partial_X_b[i][i] = occur_num* (exp_ratings[i] * sum_exprating - exp_ratings[i] * exp_ratings[i]) / sum_expr_square
            delta_L_b[i] = np.dot(partial_L_X[sample], partial_X_b[i][sample])
