...
class Discriminative():
    ...
    def updates(self, u_sample, i_sample, label_sample):
        ...
        rating = np.sum(np.multiply(self.users_embedding[u_sample], self.items_embedding[i_sample]), 1) + \
                 self.items_bias[i_sample]
        ...
        for i in range(len(u_sample)):
            delta_U[u_sample[i]] += f[i] * self.items_embedding[i_sample[i]] + self.alpha_U * self.users_embedding[
                u_sample[i]]
            delta_V[i_sample[i]] += f[i] * self.users_embedding[u_sample[i]] + self.alpha_V * self.items_embedding[
                i_sample[i]]
        ...
        """
        rating=np.dot(self.users_embedding[u],self.items_embedding[i])+self.items_bias[i]
        if rating>0:
            f=1 - label + -np.exp(-rating)/(1 + np.exp(-rating))
        else:
            f= -label + np.exp(rating)/(1+np.exp(rating))

        delta_U=f* np.array(self.items_embedding[i]) + self.alpha_U* np.array(self.users_embedding[u])
        delta_V=f* np.array(self.users_embedding[u]) + self.alpha_V* np.array(self.items_embedding[i])
        delta_Bi=f+self.beta_V * self.items_bias[i]

        ##updates simultaneously
        self.users_embedding[u]=self.users_embedding[u] - self.learning_rate * delta_U
        self.items_embedding[i]=self.items_embedding[i] - self.learning_rate * delta_V
        self.items_bias[i]=self.items_bias[i] - self.learning_rate * delta_Bi
        """
